...
| Magic Animal Carousel | ✅ PROVEN | **magic-carousel:changeAnimal** | SWC-136 | 이름 인코딩이 nextId 필드로 흘러 링 포인터 오염 |

**공개 The Ethernaut 레벨 32종을 자동 증명**한다(위 표; Delegation 파일은 프록시 본체와
...
## 모델 밖 (정직한 경계) — 유일하게 남은 1개
- **DoubleEntryPoint** — Forta **탐지 봇을 "구축"** 하는 방어형 레벨이다. 공격(익스플로잇)을
  수행하는 게 아니라 방어기를 작성하는 문제라, 본 도구(exploit-prover)의 대상 자체가 아니다.
  나머지 **공개 32개 레벨(취약 익스플로잇 전부)은 동적 자동 증명**된다.

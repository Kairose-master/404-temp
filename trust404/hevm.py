...
Official _prove warps/rolls then calls run() once. py-evm must freeze
block.number / block.timestamp to the manifest and honour warp/roll/deal/addr
at 0x7109 inside that run() — Solidity 0.8 also requires extcodesize>0 there.
"""
...
def make_backend(block_number: int, timestamp: int):
    """eth-tester PyEVMBackend with frozen NUMBER/TIMESTAMP and HEVM precompile."""

...
트랙 숨은 셋에서 정규식 IR이 죽던 구멍은 하네스 경계 안에서 닫았다
(배열·struct 생성자, 형제 게터, 스팟/윈도우 TWAP, 피해자 approve,
같은 EVM 메신저, swap-only 의도 경로 게이트).
**안 닫힌 것:** 실제 두 체인, 우리가 키를 모르는 EOA 의 오프라인 서명,
사람 스펙 없는 의도 판정. 그건 이 문서의 주제고 채점 칸이 아니다.
